...
**Local storage** – All configuration lives in the browser’s extension storage: the mapping from domains to site rules (profile plus purpose overrides), predefined profiles, the domain whitelist, Enhanced Protection state (list metadata in `enhancedLists`, heavy domain/path data in `enhancedData_{listId}` keys, active preset in `enhancedPreset`), and opt-in flags for remote sync (`dynamicListsConsent`) and the consent-enhanced link (`consentEnhancedLink`).
