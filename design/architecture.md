...
- **Enhanced Protection**: list metadata, domain/path data, active preset, and selected regional languages. 40 lists total organised in three tiers: 19 basic (auto-enabled with Balanced), 6 full (added by Full preset), and 15 optional (community lists, never auto-enabled or auto-downloaded, user must download individually). Sources include 6 ProtoConsent Core lists (one per blocking purpose plus security, maintained by the project) and third-party lists from EasyList, AdGuard, HaGeZi, OISD, Phishing Army, Blocklist Project, Steven Black, 1Hosts, and Fanboy. Regional filters cover 36 languages with per-source entries (47 sources x 2 types = 94 list entries), synthesized at runtime from `regional-languages.json`. Multi-source languages support individual source toggles. Core lists are bundled for first-install availability and updated weekly via CDN from the [data repository](https://github.com/ProtoConsent/data), where a GitHub Actions workflow refreshes all sources every Tuesday.
