...
| v0.3.0 | Enhanced Protection: 13 optional third-party enhanced lists with presets (Off/Balanced/Full/Custom), on-demand CDN fetch with consent gate, consent-enhanced link (denied purposes auto-activate matching Enhanced lists), Enhanced tab UI, enhanced scope in consent view, purposes-settings enhanced presets and toggles (sync + consent link in 2-column grid), 4-screen onboarding with enhanced opt-ins, Client Hints stripping, CNAME cloaking detection, Cookie banner detection |
