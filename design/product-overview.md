...
| v0.5.6 | Rebrand static rulesets (`block_*` -> `protoconsent_*`), reorganize CDN enhanced lists into `protoconsent/` and `external/` subdirectories, merge classifier delta into bundle (58K domains, 1202 path rules), add `protoconsent_security` list, remove 5 redundant third-party lists, CDN cache-busting fix |
