...
**Enhanced protection:** Users can optionally activate curated third-party enhanced lists for broader coverage beyond the core static rulesets. 14 lists from trusted open-source projects (EasyList, EasyPrivacy, AdGuard, HaGeZi, Steven Black, OISD, 1Hosts, Blocklist Project, AdGuard CNAME Trackers) are available, organized in three presets (Off, Balanced, Full) or with individual list control. Remote list fetching is gated behind an explicit consent toggle (opt-in during onboarding or in Purpose Settings). Lists are fetched on demand from public CDN sources and stored locally. The consent-enhanced link optionally bridges consent purposes with Enhanced lists: when a user denies a purpose (e.g. Ads), Enhanced lists matching that category activate automatically, even if the Enhanced preset is Off. The Enhanced tab in the popup provides preset selection, per-list download/toggle/remove controls, and status indicators. Enhanced blocks appear in the Log tab with a shield icon alongside category icons where applicable. One list (EasyList Cosmetic) provides element-hiding via CSS injection rather than network blocking, hiding ad containers and banners left empty after request-level blocking.
