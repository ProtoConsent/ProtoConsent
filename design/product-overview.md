...
The two modes share the same consent engine: GPC policy, CMP cookie injection, Client Hints stripping, and cosmetic hiding work identically in both modes. Only the enforcement strategy changes - Blocking mode enforces its own rules, while Monitoring mode observes blocks from external sources. For the full capabilities table and mode details, see [operating-modes.md](architecture/operating-modes.md).
...
| v0.4.2 | CMP auto-response: banner suppression for 22 CMP frameworks via cookie injection, IAB TCF v2.2 TC String, scroll unlock, CDN-based signature updates ([cmp-auto-response.md](architecture/cmp-auto-response.md)) |
