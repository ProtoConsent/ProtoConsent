...
**Network‑level enforcement:** The extension blocks network requests that correspond to disallowed purposes using the browser’s built‑in declarative rules engine. Enforcement is based on curated blocklists organized by purpose (domain‑based and path‑based), with per‑site overrides that respect the user’s profile without touching the global rules. When privacy‑relevant purposes are denied, the extension also sends a conditional [Global Privacy Control](https://globalprivacycontrol.org/) (Sec‑GPC) header.
...
- The popup provides four views accessible via a vertical tab rail: **Purposes** (purpose toggles and blocked stats), **Protection** (curated blocklists in three tiers: core, third-party, and optional), **Overview** (operating mode dashboard with signal status, purpose-attributed blocks, CMP detection, and cosmetic filtering), and **Log** (real-time request monitoring, blocked domains, GPC tracking, whitelist management, debug).
...
- When advanced tracking is denied, high-entropy Client Hints headers are stripped to reduce fingerprinting surface, with a global toggle in Purpose Settings.
- URL tracking parameters (`utm_source`, `fbclid`, `gclid`, etc.) are stripped from navigations. Stripped parameters are detected by comparing the original URL against the committed URL and displayed in the Overview tab accordion and the Log tab request stream.
...
**Enhanced protection:** Curated enhanced blocklists provide broader coverage beyond the core rulesets, enabled by default. Available lists include ProtoConsent Core purpose-based lists (updated weekly via CDN) and third-party blocking lists from trusted open-source projects, plus cosmetic filtering, CNAME detection, and regional filters covering 36 languages with language-specific blocking and cosmetic rules. Lists are organized in three presets (Off, Balanced, Full) or with individual list control. Regional lists are included in Balanced and Full presets when the user has selected at least one language. A consent-enhanced link bridges consent purposes with Enhanced lists: denying a purpose automatically activates Enhanced lists matching that category, even if the preset is Off. Cosmetic filtering hides ad containers left empty after network-level blocking. Enhanced blocks appear in the Log tab with a shield icon alongside category icons.

> Third-party list sources include EasyList, EasyPrivacy, Fanboy Social, AdGuard DNS, OISD, HaGeZi, Phishing Army, Blocklist Project, Steven Black, and 1Hosts. See [list-catalog.md](architecture/list-catalog.md) for the full catalog and current counts.
...
**Operating modes:** ProtoConsent supports two operating modes, selectable in Purpose Settings. In *Blocking* mode (default), the extension blocks requests directly using its own rulesets. In *Monitoring* mode, the extension delegates network blocking to an external blocker (such as uBlock Origin, AdGuard, or Brave Shields) and focuses on purpose attribution, GPC signaling, CMP auto-response, cosmetic filtering, and Client Hints stripping. A dedicated Overview tab in the popup shows the current mode, signal status, purpose-attributed blocks with domain detail, and CMP auto-response state. A semaphore indicator in the popup header shows the active mode at a glance.

The two modes share the same consent engine: GPC policy, CMP auto-response (cosmetic hiding, scroll unlock, experimental cookie injection), Client Hints stripping work identically in both modes. Only the enforcement strategy changes - Blocking mode enforces its own rules, while Monitoring mode observes blocks from external sources. For the full capabilities table and mode details, see [operating-modes.md](architecture/operating-modes.md).
...
**Onboarding:** A welcome page guides first-time users through two steps: (1) choosing a default privacy profile and (2) a confirmation screen with active feature summary. Enhanced protection lists are enabled by default and download automatically in the background. The extension works out of the box without requiring manual configuration.

**CMP auto-response:** Consent banners from known CMP frameworks (including IAB TCF v2.2) are hidden automatically based on the user's purpose preferences. A cosmetic CSS layer hides known banner elements, and a scroll-unlock mechanism restores normal page behaviour. An experimental cookie injection layer (disabled by default) can optionally pre-set consent cookies before CMP scripts load. For IAB TCF-compliant CMPs, the extension generates a valid TCF v2.2 Transparency and Consent String. See [cmp-auto-response.md](architecture/cmp-auto-response.md).

## 4. High-level architecture

ProtoConsent is implemented as a browser extension that runs entirely on the client side. The extension has two main parts: a popup user interface that lets people inspect and change their settings for the current site, and a background service worker that stores rules, updates browser configuration, and applies enforcement through built-in blocking APIs. A lightweight SDK (*protoconsent.js*) and a content script bridge allow websites and consent tools to read the user’s browser-level choices directly from the page context.

At its core, ProtoConsent maintains a small data structure mapping each domain to a rule that combines a profile with purpose-level overrides. Each purpose resolves to "allowed" or "denied". Rules are stored locally by the extension, so that the browser can apply them even when the user is offline. When a user changes the profile or toggles a purpose in the popup, the background updates the stored rule and rebuilds the blocking configuration. As a result, subsequent network requests from that site that match a disabled purpose are blocked by the browser before reaching the remote server.

The user opens a website and, if needed, adjusts the profile or individual purposes for that site in the ProtoConsent popup. The extension updates the local rule for that domain and refreshes the browser’s internal configuration so that it reflects the new choices. When the page (or its embedded third-party scripts) later tries to contact external services, those network requests are checked against the active rules: requests associated with disabled purposes are blocked, while requests associated with allowed purposes continue as normal. This keeps the enforcement logic close to the user, using only standard browser capabilities.
...
| Version | Features |
|---------|----------|
| v0.1.0 | Purpose-based consent model, per-domain rules, network-level enforcement (global + per-site), popup UI with profile and purpose toggles, [JavaScript SDK](../sdk/protoconsent.js) with [TypeScript declarations](../sdk/protoconsent.d.ts) |
| v0.1.1 | Path-based blocking rules, blocked request counter, conditional [GPC signal](https://globalprivacycontrol.org/), [.well-known side panel](spec/protoconsent-well-known.md) with [Consent Commons](https://consentcommons.com/) icons, debug panel |
| v0.2.0 | Curated blocklists ([list-catalog.md](architecture/list-catalog.md)), onboarding page, purpose settings page, [demo site](https://demo.protoconsent.org) |
...
| v0.3.0 | Enhanced Protection: optional third-party lists with presets (Off/Balanced/Full/Custom), CDN fetch with consent gate, consent-enhanced link, cosmetic filtering, Client Hints stripping, CNAME cloaking detection, cookie banner detection |
| v0.4.0 | Inter-extension messaging API ([spec](spec/inter-extension-protocol.md)), ProtoConsent Core lists (purpose-based, weekly CDN updates via [GitHub Actions](https://github.com/ProtoConsent/data)) |
| v0.4.1 | Fixes: HTTP cache bypass for list fetch, Core download side-effects. Improved: GPC tooltip, accessibility |
| v0.4.2 | CMP auto-response: banner hiding for 22 CMP frameworks via cosmetic CSS, experimental cookie injection, IAB TCF v2.2 TC String, scroll unlock, CDN-based signature updates ([cmp-auto-response.md](architecture/cmp-auto-response.md)) |
| v0.4.3 | CMP banner detection (~285 detectors), localStorage consent observation, cookie consent decoders (OneTrust, Cookiebot, CookieYes, Complianz, Wix), security hardening |
| v0.5.0 | Operating modes (Blocking/Monitoring), Overview tab dashboard, URL parameter stripping, blocker detection, block provenance |
| v0.5.1 | Regional filter lists (13 languages), popup UI refactor (tabs renamed to Purposes/Protection/Overview/Log), bug fixes |
...
| v0.5.5 | GPC display consistency across all tabs, dark mode badge and dark icon set, CMP signature updates (LinkedIn, Google), Log and Overview UI fixes |
| v0.5.6 | Chrome Web Store and Edge Add-ons publication. Expanded curated blocklists across 6 purpose categories, new security protection category, streamlined Enhanced catalog, CDN cache-busting for list updates, Protection tab visual refresh |
| v0.6 | Simplified onboarding (2 steps), enhanced protection on by default with automatic list refresh, regional filters expanded to 30 languages. EasyList Cookie and Web Annoyances lists, Hotfix list with observability, per-selector cosmetic whitelist. Inter-extension API classify action, unmatched domain heuristic attribution, lifetime blocked counter. Brave Shields detection, site-specific CMP signatures (excludeHosts), .well-known click-to-recheck |
