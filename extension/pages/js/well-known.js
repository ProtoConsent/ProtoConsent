...
// --- TCF CMP info rendering ---

// Known IAB CMP IDs → names (most common CMPs)
const CMP_NAMES = {
...
function loadTcfInfo() {
  const container = document.getElementById("pc-tcf-declaration");
  if (!container || !currentDomain) return;
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    chrome.runtime.sendMessage({ type: "PROTOCONSENT_GET_TCF", tabId: tabs[0].id }, (resp) => {
      ...
      renderTcfInfo(container, resp.tcf);
    });
  });
}
...
function renderTcfInfo(container, tcf) {
  ...
  cmpEl.className = "pc-tcf-provider";
  const cmpName = tcf.cmpId && CMP_NAMES[tcf.cmpId]
    ? "Managed by " + CMP_NAMES[tcf.cmpId]
    : "Cookie consent manager detected";
  cmpEl.textContent = cmpName;
  container.appendChild(cmpEl);

  // Purpose consents grid (only if populated)
  if (tcf.purposeConsents && Object.keys(tcf.purposeConsents).length > 0) {
    const IAB_PURPOSES = {
    ...
    for (const [id, consented] of Object.entries(tcf.purposeConsents)) {
      const row = document.createElement("div");
      row.className = "pc-tcf-purpose-row";
      const check = document.createElement("span");
      check.className = consented ? "pc-tcf-accepted" : "pc-tcf-denied";
      check.textContent = consented ? "\u2713" : "\u2717";
      const label = document.createElement("span");
      label.textContent = IAB_PURPOSES[id] || ("Purpose " + id);
  ...
  // Now that TCF content is rendered, ensure the TCF pill is clickable
  // (updateTcfIndicator may have run before content was ready)
  const tcfPill = document.getElementById("pc-tcf-indicator");
  if (tcfPill && tcfPill.classList.contains("is-active") && !tcfPill._tcfClickBound) {
    tcfPill.style.cursor = "pointer";
    tcfPill.setAttribute("role", "button");
    tcfPill.setAttribute("tabindex", "0");
    tcfPill.addEventListener("click", toggleSidePanel);
    tcfPill.addEventListener("keydown", (e) => {
      if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
        toggleSidePanel();
      }
    });
    tcfPill._tcfClickBound = true;
  }
}
